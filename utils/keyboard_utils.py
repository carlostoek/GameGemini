...
def get_admin_main_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📣 Enviar Post con Reacciones", callback_data="admin_send_channel_post_reactions")] # <-- NUEVO BOTÓN
    ])
    return keyboard

def get_reaction_keyboard(message_id: int):
    """
    Genera un teclado inline con botones de reacción para un mensaje de canal.
    message_id es el ID del mensaje en el canal al que se refieren estas reacciones.
    """
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="Me identifico 🙋‍♀️", callback_data=f"react_{message_id}_reflect"),
            InlineKeyboardButton(text="Me inspira 💡", callback_data=f"react_{message_id}_inspire")
        ],
        [
            InlineKeyboardButton(text="Me emociona 🥰", callback_data=f"react_{message_id}_emotion"),
            InlineKeyboardButton(text="Me hace pensar 🤔", callback_data=f"react_{message_id}_think")
        ]
    ])
    return keyboard
